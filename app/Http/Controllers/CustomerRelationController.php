    ...
    public function crmAnalysisResult(Request $request)
    {
        if ($request->ajax()) {
            if (!empty($request->from_date) && !empty($request->to_date)) {
                ...
                $total['bandwidth'] = DB::table('customer_relations')
                    ->where('customer_relations.created_at', '>', $from)
                    ->where('customer_relations.created_at', '<', $to)
                    ->where('customer_relations.client_type', 'bandwidth')
                    ->count();
                $total['mac'] = DB::table('customer_relations')
                    ->where('customer_relations.created_at', '>', $from)
                    ->where('customer_relations.created_at', '<', $to)
                    ->where('customer_relations.client_type', 'mac')
                    ->count();
                $total['corporate'] = DB::table('customer_relations')
                    ->where('customer_relations.created_at', '>', $from)
                    ->where('customer_relations.created_at', '<', $to)
                    ->where('customer_relations.client_type', 'corporate')
                    ->count();
                $total['local'] = DB::table('customer_relations')
                    ->where('customer_relations.created_at', '>', $from)
                    ->where('customer_relations.created_at', '<', $to)
                    ->where('customer_relations.client_type', 'local')
                    ->count();
                $total['blimit'] = DB::table('crm_work_limits')
                    ->select('crm_work_limits.blimit')->first();
                $total['mlimit'] = DB::table('crm_work_limits')
                    ->select('crm_work_limits.mlimit')->first();
                $total['climit'] = DB::table('crm_work_limits')
                    ->select('crm_work_limits.climit')->first();
                $total['llimit'] = DB::table('crm_work_limits')
                    ->select('crm_work_limits.llimit')->first();
                //    }
                //  }
                // dd($total);
                $different_days = $from->diffInDays($to); //
                return view('admin.crm.analysisResult', ['total' => $total, 'different_days' => $different_days, 'from' => $from, 'to' => $to]);
            }
        }
    }
}